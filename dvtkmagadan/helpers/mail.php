...
$message .= '</tbody></table>';
//Отправка осуществляется через запрос POST

//Отправка форм установка почт от куда отправляется и куда отправляется
if ($reqStatus){
    $headers = "From: oksana.shleyfman@yandex.ru \r\n" .
        "Reply-To: channel.my.honey@gmail.com \r\n" .
        'X-Mailer: PHP/' . phpversion();
    $headers .= 'MIME-Version: 1.0' . "\r\n";
    $headers .= 'Content-type: text/html; charset=utf-8' . "\r\n";
    if (mail('magnet49@yandex.ru, info@telemagadan.ru', 'Заявка с сайта', $message, $headers)){
        echo $reqStatus;
    } else {
        echo false;
    }
